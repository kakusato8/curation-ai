          ...
          try {
            ...
            const model = this.genAI.getGenerativeModel({ 
              ...
              systemInstruction: {
                role: 'system',
                parts: [{
                  text: `あなたは最新情報検索に特化したAIアシスタントです。以下の指示に従ってください：

1. 質問に関する最新情報を必ずGoogle検索を使用して取得する
2. 株価、金融データ、ニュース、統計情報については特に正確性を重視する
3. 検索結果に基づいて信頼性の高い情報のみを使用する
4. 不確実な情報については推測ではなく「不明」と明記する
5. 日本語で読みやすく構造化された形式で回答する
6. ユーザーの質問内容は見出しに含めない（冗長性を避けるため）
7. 自然で読みやすいコンテンツを作成する`
                }]
              }
            });
...
上記について最新情報をGoogle検索で取得し、以下の形式で回答してください：
...
**注意**: この情報は検索時点（${new Date().toLocaleString('ja-JP')}）のものです。`;
            ...
            if (response.candidates && response.candidates[0]) {
              ...
              if (groundingMetadata) {
                this.logger.log(`Grounding metadata available with ${groundingMetadata.searchEntryPoints?.length || 0} search entries`);
  ...
  private formatGroundingMetadata(metadata: any): string {
    if (!metadata.searchEntryPoints) return '';
  ...
  private async generateContentWithoutGrounding(query: string): Promise<string> {
    try {
      const model = this.genAI.getGenerativeModel({ 
...
5. 推測や古い情報は使用しない`
            ...
            // グラウンディングメタデータの確認
            const groundingMetadata = response.candidates?.[0] ? (response.candidates[0] as any).groundingMetadata : null;
            const hasGrounding = groundingMetadata?.searchEntryPoints?.length > 0;
            ...
            return {
              content: text,
              ...(includeMetadata && hasGrounding && { groundingMetadata })
            };
  ...
  // 株価やリアルタイムデータ専用のメソッド
  async getRealtimeData(symbol: string, dataType: 'stock' | 'crypto' | 'forex' | 'news' = 'stock'): Promise<string> {
    const queries = {
